...
Aisla dependencias por proyecto. Créalo al inicio, actívalo al trabajar y ciérralo al salir.\\
\textbf{Crear}: \texttt{Abre dir $\rightarrow$ Ctrl+Ñ $\rightarrow$ python -m venv .venv $\rightarrow$ Ctrl+Shift+P $\rightarrow$ Python Interpreter $\rightarrow$ .venv}\\
\textbf{Auto-Activar}: \texttt{PowerShell Admin $\rightarrow$ Set-ExecutionPolicy RemoteSigned -Scope CurrentUser}\\
En \texttt{VS Code (Ctrl+,)} marcar $\rightarrow$ \texttt{Python: Terminal: Activate Environment}\\
\hypertarget{pip_venv}{
\text{Actualizar Pip}}: \texttt{python -m pip install --upgrade pip} \textit{(Ejecutar tras activar el venv)}\\
\text{Verificar Ruta}: \texttt{where pip} debe apuntar a la carpeta \texttt{.venv}\\
\text{Formato}: \texttt{pip install black $\rightarrow$ Ctrl+, $\rightarrow$ Python Formatting Provider $\rightarrow$ black}\\
\text{Básicos}: \texttt{pip install matplotlib numpy scipy pandas scikit-learn jupyter}\\
\text{Persistencia}: \texttt{pip freeze > requirements.txt} \textit{(Documenta versiones)}\\
\textbf{Activar Manual}: \texttt{.venv\textbackslash Scripts\textbackslash activate} \;\;\; \textbf{Desactivar}: \texttt{deactivate}\\ 
